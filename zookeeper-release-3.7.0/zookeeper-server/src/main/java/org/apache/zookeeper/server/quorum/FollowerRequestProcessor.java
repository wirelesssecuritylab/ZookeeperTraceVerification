    ...
    public void run() {
        try {
            while (!finished) {
                ...
                case OpCode.multi:
                case OpCode.check:
                    zks.getFollower().request(request);//1022 以上枚举的请求都会被转发到服务器中
                    LOG.debug("1022 follower forword request to leader,request={},zxid={},sessionId={}",request.toString(),Long.toHexString(request.zxid),Long.toHexString(request.sessionId));
